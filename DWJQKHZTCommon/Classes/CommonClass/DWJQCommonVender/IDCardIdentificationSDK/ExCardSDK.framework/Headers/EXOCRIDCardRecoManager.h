...
/**
 ...
 @param imageBuffer 视频帧
 @param bufferCompletedBlock 识别结果回调
 */
-(void)getIDCardInfoWithImageBuffer:(CVImageBufferRef)imageBuffer OnCompleted:(EXOCRCompletedIDCardBlock)bufferCompletedBlock;

/**
 是否使用四边定位算法，优化对少数民族身份证的识别
 
 @param bPromote 默认为NO，YES为使用四边定位算法；NO不使用四边定位算法
 */
- (void)setPromoteMinoritiesIDCard:(BOOL)bPromote;
/**
 是否在扫描完成后自动振动，以提示用户扫描完成
 
 @param bAudio 默认为YES，YES为自动振动；NO为不振动
 */
- (void)setPlaySystemSound:(BOOL)bAudio;
...
/**
继续识别时，设置的默认超时时间是否有效（仅在设置自定义扫描视图成功后生效）
 
@param bAble 默认超时时间是否有效（默认为NO）
*/
-(void)recoTimeoutWithContinueReco:(BOOL)bAble;
/**
...
/**
 ...
 @return 当前的帧图片，在识别成功的回调里调用，返回当前识别成功的原图
 */
