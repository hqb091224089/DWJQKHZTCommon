...
#import "DWJQSVIndefiniteAnimatedView.h"
...
@property (nonatomic, strong) DWJQSVRadialGradientLayer *backgroundRadialGradientLayer;
...
@property (nonatomic, strong) DWJQSVProgressAnimatedView *ringView;
@property (nonatomic, strong) DWJQSVProgressAnimatedView *backgroundRingView;
...
@implementation DWJQSVProgressHUD {
    BOOL _isInitializing;
}

+ (DWJQSVProgressHUD*)sharedView {
    static dispatch_once_t once;
    
    static DWJQSVProgressHUD *sharedView;
...
+ (void)setDefaultStyle:(DWJQSVProgressHUDStyle)style {
    [self sharedView].defaultStyle = style;
}

+ (void)setDefaultMaskType:(DWJQSVProgressHUDMaskType)maskType {
    [self sharedView].defaultMaskType = maskType;
}

+ (void)setDefaultAnimationType:(DWJQSVProgressHUDAnimationType)type {
...
+ (void)showWithMaskType:(DWJQSVProgressHUDMaskType)maskType {
    DWJQSVProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
...
+ (void)showWithStatus:(NSString*)status maskType:(DWJQSVProgressHUDMaskType)maskType {
    DWJQSVProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress maskType:(DWJQSVProgressHUDMaskType)maskType {
    DWJQSVProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress status:(NSString*)status maskType:(DWJQSVProgressHUDMaskType)maskType {
    DWJQSVProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showInfoWithStatus:(NSString*)status maskType:(DWJQSVProgressHUDMaskType)maskType {
    DWJQSVProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showSuccessWithStatus:(NSString*)status maskType:(DWJQSVProgressHUDMaskType)maskType {
    DWJQSVProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showErrorWithStatus:(NSString*)status maskType:(DWJQSVProgressHUDMaskType)maskType {
    DWJQSVProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showImage:(UIImage*)image status:(NSString*)status maskType:(DWJQSVProgressHUDMaskType)maskType {
    DWJQSVProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        NSBundle *bundle = [NSBundle bundleForClass:[DWJQSVProgressHUD class]];
        NSURL *url = [bundle URLForResource:@"DWJQSVProgressHUD" withExtension:@"bundle"];
...
- (void)showProgress:(float)progress status:(NSString*)status {
    __weak DWJQSVProgressHUD *weakSelf = self;
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        __strong DWJQSVProgressHUD *strongSelf = weakSelf;
        if(strongSelf){
...
- (UIView*)indefiniteAnimatedView {
    ...
    if(self.defaultAnimationType == SVProgressHUDAnimationTypeFlat){
        ...
        if(_indefiniteAnimatedView && ![_indefiniteAnimatedView isKindOfClass:[DWJQSVIndefiniteAnimatedView class]]){
            [_indefiniteAnimatedView removeFromSuperview];
            _indefiniteAnimatedView = nil;
        }
        
        if(!_indefiniteAnimatedView){
            _indefiniteAnimatedView = [[DWJQSVIndefiniteAnimatedView alloc] initWithFrame:CGRectZero];
        }
        
        // Update styling
        DWJQSVIndefiniteAnimatedView *indefiniteAnimatedView = (DWJQSVIndefiniteAnimatedView*)_indefiniteAnimatedView;
...
- (DWJQSVProgressAnimatedView*)ringView {
    if(!_ringView) {
        _ringView = [[DWJQSVProgressAnimatedView alloc] initWithFrame:CGRectZero];
    }
...
- (DWJQSVProgressAnimatedView*)backgroundRingView {
    if(!_backgroundRingView) {
        _backgroundRingView = [[DWJQSVProgressAnimatedView alloc] initWithFrame:CGRectZero];
...
-(UIView *)backgroundView {
    ...
    if(self.defaultMaskType == SVProgressHUDMaskTypeGradient){
        if(!_backgroundRadialGradientLayer){
            _backgroundRadialGradientLayer = [DWJQSVRadialGradientLayer layer];
        }
...
- (void)setDefaultAnimationType:(DWJQSVProgressHUDAnimationType)animationType {
