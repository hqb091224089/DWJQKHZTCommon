...
#ifdef ISAppStore
//对应SoochowShowcaiAppstore
static const char kKeyChainUDIDAccessGroup[] = "F7S3SN9648.com.Soochow.ShowCaiAppstore";
#else
//对应com.Soochow.ShowCai
static const char kKeyChainUDIDAccessGroup[] = "VSU9JKR7LV.com.Soochow.ShowCaiAppstore";
#endif
...
+ (NSString*)_ADID
{
    ...
    //这里还要加一个库。AdSupport.framework
     NSString *adids = [[[ASIdentifierManager sharedManager] advertisingIdentifier] UUIDString];
...
//+ (NSString*)getADIDFromKeyChain
//{
//    NSMutableDictionary *dictForQuery = [[NSMutableDictionary alloc] init];
//    [dictForQuery setValue:(id)kSecClassGenericPassword forKey:(id)kSecClass];
//    
//    // set Attr Description for query
//    [dictForQuery setValue:[NSString stringWithUTF8String:kKeychainADIDItemIdentifier]
//                    forKey:kSecAttrDescription];
//    
//    // set Attr Identity for query
//    NSData *keychainItemID = [NSData dataWithBytes:kKeychainADIDItemIdentifier
//                                            length:strlen(kKeychainADIDItemIdentifier)];
//    [dictForQuery setObject:keychainItemID forKey:(id)kSecAttrGeneric];
//    
//    // The keychain access group attribute determines if this item can be shared
//    // amongst multiple apps whose code signing entitlements contain the same keychain access group.
//    NSString *accessGroup = [NSString stringWithUTF8String:kKeyChainADIDAccessGroup];
//    if (accessGroup != nil)
//    {
//#if TARGET_IPHONE_SIMULATOR
//        // Ignore the access group if running on the iPhone simulator.
...
//    
//    if (queryErr == errSecItemNotFound) {
//        NSLog(@"KeyChain Item: %@ not found!!!", [NSString stringWithUTF8String:kKeychainADIDItemIdentifier]);
//    }
//    else if (queryErr != errSecSuccess) {
...
//}
//
//+ (BOOL)settADIDToKeyChain:(NSString*)udid
//{
//    NSMutableDictionary *dictForAdd = [[NSMutableDictionary alloc] init];
//    
//    [dictForAdd setValue:(id)kSecClassGenericPassword forKey:(id)kSecClass];
//    [dictForAdd setValue:[NSString stringWithUTF8String:kKeychainADIDItemIdentifier] forKey:kSecAttrDescription];
//    
//    [dictForAdd setValue:@"ADID" forKey:(id)kSecAttrGeneric];
//    
//    // Default attributes for keychain item.
//    [dictForAdd setObject:@"" forKey:(id)kSecAttrAccount];
//    [dictForAdd setObject:@"" forKey:(id)kSecAttrLabel];
...
//    
//    OSStatus writeErr = noErr;
//    if ([SvUDIDTools getADIDFromKeyChain]) {        // there is item in keychain
//        [SvUDIDTools updateADIDInKeyChain:udid];
//        [dictForAdd release];
//        return YES;
//    }
//    else {          // add item to keychain
...
//}
//
//+ (BOOL)removeADIDFromKeyChain
//{
//    NSMutableDictionary *dictToDelete = [[NSMutableDictionary alloc] init];
//    
//    [dictToDelete setValue:(id)kSecClassGenericPassword forKey:(id)kSecClass];
//    
//    NSData *keyChainItemID = [NSData dataWithBytes:kKeychainADIDItemIdentifier length:strlen(kKeychainADIDItemIdentifier)];
//    [dictToDelete setValue:keyChainItemID forKey:(id)kSecAttrGeneric];
//    
//    OSStatus deleteErr = noErr;
//    deleteErr = SecItemDelete((CFDictionaryRef)dictToDelete);
...
//}
//
//+ (BOOL)updateADIDInKeyChain:(NSString*)newUDID
//{
//    
//    NSMutableDictionary *dictForQuery = [[NSMutableDictionary alloc] init];
//    
...
//        
//        NSMutableDictionary *dictForUpdate = [[NSMutableDictionary alloc] init];
//        [dictForUpdate setValue:[NSString stringWithUTF8String:kKeychainADIDItemIdentifier] forKey:kSecAttrDescription];
//        [dictForUpdate setValue:keychainItemID forKey:(id)kSecAttrGeneric];
//        
//        const char *udidStr = [newUDID UTF8String];
//        NSData *keyChainItemValue = [NSData dataWithBytes:udidStr length:strlen(udidStr)];
