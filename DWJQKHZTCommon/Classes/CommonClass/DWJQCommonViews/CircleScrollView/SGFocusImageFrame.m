...
- (void)setupViews
{
    ITEM_WIDTH = self.bounds.size.width;
    ...
    _pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, self.bounds.size.height -16-10, self.bounds.size.width, 10)];
    _pageControl.center = CGPointMake(self.bounds.size.width/2.0, self.bounds.size.height -16-5);
...
-(void)addImageViews:(NSArray *)aImageItems{
    ...
    float space = 0;
    CGSize size = CGSizeMake(self.bounds.size.width, 0);
    for (int i = 0; i < aImageItems.count; i++) {
        SGFocusImageItem *item = [aImageItems objectAtIndex:i];
        UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(i * _scrollView.bounds.size.width+space, space, _scrollView.bounds.size.width-space*2, _scrollView.bounds.size.height-2*space-size.height)];
        ...
        {
            [imageView setImage:item.imageData];
        }
        ...
        CGRect titleRect = [_titleLabel.text boundingRectWithSize:CGSizeMake(self.bounds.size.width-20, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName :_titleLabel.font} context:nil];
        _titleLabel.frame = CGRectMake(20+i * _scrollView.bounds.size.width+space, 16, titleRect.size.width, titleRect.size.height);
        [_scrollView addSubview:_titleLabel];
        [_titleLabel release];
        
    }
    _scrollView.contentSize = CGSizeMake(_scrollView.bounds.size.width * aImageItems.count, _scrollView.bounds.size.height);
    _pageControl.numberOfPages = aImageItems.count>1?aImageItems.count -2:aImageItems.count;
    _pageControl.currentPage = 0;
    
    _pageControl.frame = CGRectMake(0, self.bounds.size.height -16,16*aImageItems.count, 16);
    _pageControl.center = CGPointMake(self.bounds.size.width/2.0, self.bounds.size.height -8);
...
-(void)stopAutoScroll
{
    [SGFocusImageFrame cancelPreviousPerformRequestsWithTarget:self];
}
...
- (void)switchFocusImageItems
{
    ...
    CGFloat targetX = _scrollView.contentOffset.x + _scrollView.bounds.size.width;
    NSArray *imageItems = objc_getAssociatedObject(self, (const void *)SG_FOCUS_ITEM_ASS_KEY);
...
- (void)singleTapGestureRecognizer:(UIGestureRecognizer *)gestureRecognizer
{
    ...
    NSArray *imageItems = objc_getAssociatedObject(self, (const void *)SG_FOCUS_ITEM_ASS_KEY);
    int page = (int)(_scrollView.contentOffset.x / _scrollView.bounds.size.width);
