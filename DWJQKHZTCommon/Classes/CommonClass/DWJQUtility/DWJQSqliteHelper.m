...
@interface DWJQSqliteHelper(PrivateMethods)
-(void)open;
...
@implementation DWJQSqliteHelper
...
{
    return  [[DWJQSqliteHelper alloc]initWithMigrations];
}
...
-(id)initWithMigrations:(BOOL)loggingEnabled {
    self = [self initWithFileName:kDatabaseName];
    if (!self) { return nil; }
    
    [self setLogging:loggingEnabled];
    [self runMigrations];
    [DWJQBaseDBData setDatabase:self];
